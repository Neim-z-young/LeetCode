...
public class Solution {
    ...
    public List<List<String>> groupAnagrams(String[] strs) {
        ...
        for (int i=0 ;i<strs.length; i++){
            if(group[i]==0){
                ...
                for(int k = 0; k<strs[i].length(); k++){
                    flag[strs[i].charAt(k)-'a']++;
                }
                //计算同组的字符串
                for(int j=i+1; j<strs.length; j++){
                    int[] flag2 = new int[26];
                    int k = 0;
                    //防止strs[j]是strs[i]的前缀
                    if(strs[i].length()!=strs[j].length()){
                        continue;
                    }
                    for(; k<strs[j].length(); k++){
                        if(flag[strs[j].charAt(k)-'a']<(++flag2[strs[j].charAt(k)-'a'])){
                            break;
                        }
                    }
                    if(k==strs[j].length()){
        ...
        for(int i=0; i<strs.length; i++){
            ans.get(group[i]-1).add(strs[i]);
        }
        return ans;
    }

    //要实现速度快的，还是得使用hashMap或集合
}